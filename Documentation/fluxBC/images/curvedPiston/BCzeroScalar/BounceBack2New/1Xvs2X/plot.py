...
os.chdir('../../1X/')
import curvedPiston1X as case0
os.chdir('../2X/')
import curvedPiston2X as case1
os.chdir('../plots/1Xvs2X')

fig = plt.figure()
plt.axes([0.17,0.18,0.97-0.17,0.92-0.18])
plt.plot(case0.time, case0.massError,'-', color= 'red', label=cases[0])
plt.plot(case1.time, case1.massError,'-', color= 'black', label=cases[1])
plt.grid(b=True, which='major',  axis='y', linewidth=1)
#plt.xlim(0,750)
plt.xlabel('Time (s)')
plt.ylabel('Mass Loss Error (\%)')
plt.legend(loc=0)
plt.savefig('massError.png')
